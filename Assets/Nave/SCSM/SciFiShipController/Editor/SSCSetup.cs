...
namespace SciFiShipController
{
    ...
    public static class SSCSetup
    {
        ...
        /// <summary>
        /// Is a package currently being installed?
        /// </summary>
        public static bool IsInstallingPackage { get => isInstallingPackage; }

        /// <summary>
        /// Name of the current package being installed
        /// </summary>
        public static string CurrentPackage { get => currentPackage; }
        ...
        private static bool isInstallingPackage = false;
        private static string currentPackage = string.Empty;
        ...
        private static void DefineSymbols()
        {
            ...
            const string SCSM_XR_Define = "SCSM_XR";  // This seems to now be always defined
            const string SCSM_XRM_Define = "SCSM_XRM"; // U2022, U6 Only when XR Management installed
            ...
            // v1.5.7 Contains(SSC_Define) will also find SSC_SSCXP1, XP2 etc
            // SSC XP3 also needs SCSM_XRM as SCSM_XR now always succeeds...
            string[] splitDefines = defines.Split(';');

            #region SSC
            if (!Array.Exists(splitDefines, s => s.ToUpper() == SSC_Define))
            ...
            // This now seems to always be defined
            try
            {
                ...
                if (xrDeviceType != null && !Array.Exists(splitDefines, s => s.ToUpper() == SCSM_XR_Define))
                {
                    if (string.IsNullOrEmpty(defines)) { defines = SCSM_XR_Define; }
                    else if (!defines.EndsWith(";")) { defines += ";" + SCSM_XR_Define; }

                    // SCSM is only going to support 2022.3+ although some providers may work in 2019.4 too.
            ...
            // This should be more reliable than SCSM_XR.
            if (!Array.Exists(splitDefines, s => s.ToUpper() == SCSM_XRM_Define) && IsPackageInstalled("com.unity.xr.management"))
            {
                if (string.IsNullOrEmpty(defines)) { defines = SCSM_XRM_Define; }
                else if (!defines.EndsWith(";")) { defines += ";" + SCSM_XRM_Define; }
        ...
        /// <summary>
        /// Attempt to start installing a Package with PackageManager.
        /// NOTE: This is designed to only install a single package.
        /// USAGE: InstallPackage("com.unity.inputsystem");
        /// </summary>
        /// <param name="packagePath"></param>
        public static void InstallPackage(string packagePath)
        {
            if (string.IsNullOrEmpty(GetPackageVersion(packagePath)))
            {
                addRequest = UnityEditor.PackageManager.Client.Add(packagePath);
        ...
        /// <summary>
        /// This is called automatically after InstallPackage is run.
        /// NOTE: This is designed to only install one package.
        /// </summary>
        public static void InstallPackageProgress()
        {
            if (addRequest.IsCompleted)
            {
                if (addRequest.Status == UnityEditor.PackageManager.StatusCode.Success)
                {
                    Debug.Log("SSCSetup.InstallPackageProgress Installed: " + addRequest.Result.packageId);
                }
                else if (addRequest.Status >= UnityEditor.PackageManager.StatusCode.Failure)
                {
                    Debug.Log("SSCSetup.InstallPackageProgress - package install failed: " + currentPackage);
                }
        ...
        /// <summary>
        /// Is the package installed in the project?
        /// USAGE: if (!IsPackageInstalled("com.unity.textmeshpro") { }
        /// </summary>
        /// <param name="packageName"></param>
        /// <returns></returns>
        public static bool IsPackageInstalled(string packageId)
        {
            string packagePath = "Packages/" + packageId + "/package.json";

            UnityEditor.PackageManager.PackageInfo pInfo = UnityEditor.PackageManager.PackageInfo.FindForAssetPath(packagePath);

            //Debug.Log("package: " + (pInfo != null ? pInfo.name + " " + pInfo.version : packageId + " is not installed"));

            return pInfo != null;
        }

        /// <summary>
        /// Check if Text Mesh Pro Essentials has been installed in the project.
        /// </summary>
        /// <returns></returns>
        public static bool IsTextMeshProEssentialsImported()
        {
            // The Unity TerrainDemoScene_URP asset from the UAS has its own TMP folder (note the folder name is missing the space).
            // If it is installed before LBS, resources won't be placed in the default location.
            return File.Exists("Assets/TextMesh Pro/Resources/TMP Settings.asset") || File.Exists("Assets/TerrainDemoScene_URP/TextMeshPro/Resources/TMP Settings.asset");
        }
