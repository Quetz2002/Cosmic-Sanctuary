...
namespace SciFiShipController
{
    ...
    public class DemoSSCSequenceRenderers : MonoBehaviour
    {
        ...
        /// <summary>
        /// Is the group currently on?
        /// </summary>
        /// <param name="groupNumber"></param>
        /// <returns></returns>
        public bool IsGroupOn (int groupNumber)
        {
            if (isInitialised && groupNumber > 0 && groupNumber < 4)
            {
                return isGroupValid[groupNumber - 1] && isGroupActive[groupNumber - 1];
            }
            else { return false; }
        }
        ...
        {
            // Cancel any potential pending delayed deactivate methods
            if (groupNumber == 1 && group1DeactivateDelay > 0f)
            {
                CancelInvoke(nameof(DisableGroup1));
            }
            else if (groupNumber == 2 && group2DeactivateDelay > 0f)
            {
                CancelInvoke(nameof(DisableGroup2));
            }
            else if (groupNumber == 3 && group3DeactivateDelay > 0f)
            {
                CancelInvoke(nameof(DisableGroup3));
            }

            if (groupNumber == 1 && group1ActivateDelay > 0f)
            {
                Invoke(nameof(EnableGroup1), group1ActivateDelay);
            }
            else if (groupNumber == 2 && group2ActivateDelay > 0f)
            {
                Invoke(nameof(EnableGroup2), group2ActivateDelay);
            }
            else if (groupNumber == 3 && group3ActivateDelay > 0f)
            {
                Invoke(nameof(EnableGroup3), group3ActivateDelay);
            }
            else
            {
        ...
        {
            // Cancel any potential pending delayed activate methods
            if (groupNumber == 1 && group1ActivateDelay > 0f)
            {
                CancelInvoke(nameof(EnableGroup1));
            }
            else if (groupNumber == 2 && group2ActivateDelay > 0f)
            {
                CancelInvoke(nameof(EnableGroup2));
            }
            else if (groupNumber == 3 && group3ActivateDelay > 0f)
            {
                CancelInvoke(nameof(EnableGroup3));
            }

            if (groupNumber == 1 && group1DeactivateDelay > 0f)
            {
                Invoke(nameof(DisableGroup1), group1DeactivateDelay);
            }
            else if (groupNumber == 2 && group2DeactivateDelay > 0f)
            {
                Invoke(nameof(DisableGroup2), group2DeactivateDelay);
            }
            else if (groupNumber == 3 && group3DeactivateDelay > 0f)
            {
                Invoke(nameof(DisableGroup3), group3DeactivateDelay);
            }
            else
            {
