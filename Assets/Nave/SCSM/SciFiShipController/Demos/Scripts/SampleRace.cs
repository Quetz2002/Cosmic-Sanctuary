...
namespace SciFiShipController
{
    ...
    public class SampleRace : MonoBehaviour
    {
        ...
        /// <summary>
        /// Set the AIState to idle when the last path is completed
        /// </summary>
        public bool idleWhenFinished = false;
        ...
        public void FinishedPathCallback (ShipAIInputModule shipAIInputModule)
        {
            ...
            // We have finished all of the paths
            if (currentRacePathIndex >= racePathsList.Count)
            {
                if (idleWhenFinished)
                {
                    shipAIInputModule.SetState(AIState.idleStateID);
                    shipAIInputModule.AssignTargetPath(null);
                    currentRacePathIndex = -1;
                    Debug.Log("SampleRace: " + shipAIInputModule.name + " has finished all the paths");
                }
                else
                {
                    // Loop back to the first path, assuming they complete a circuit
                    currentRacePathIndex = 0;
                }
            }

            if (currentRacePathIndex >= 0)
            {
