...
namespace SciFiShipController
{
    ...
    public class PlayerInputModule : MonoBehaviour
    {
        ...
        /// <summary>
        /// Used with UnityXR input.
        /// </summary>
        public enum XRTrackingOriginMode
        {
            NotSet = 0,
            Device = 1,
            Floor = 2
        ...
        /// <summary>
        /// Attempt to recentre the XR headset when it is initialised
        /// </summary>
        [SerializeField] private bool isRecentreOnInitXR = false;

        /// <summary>
        /// For flight sitting position, set to Device and move the XR Camera to the initial starting position within
        /// the ship at design time in the editor. Default to floor for backward compatibility.
        /// </summary>
        [SerializeField] private XRTrackingOriginMode xrTrackingOriginMode = XRTrackingOriginMode.Floor;

        /// <summary>
        /// The parent transform of the XR Camera. E.g., XRCameraOffset
        /// </summary>
        [NonSerialized] private Transform xrCameraOffset = null;
        /// <summary>
        /// The initial local space position of the XR Camera parent transform. E.g., XRCameraOffset
        /// </summary>
        [NonSerialized] private Vector3 initialXRCamOffsetPos = Vector3.zero;
        /// <summary>
        /// The initial local space rotation of the XR Camera parent transform. E.g., XRCameraOffset
        /// </summary>
        [NonSerialized] private Quaternion initialXRCamOffsetRot = Quaternion.identity;
        /// <summary>
        /// The parent transform of the XR Camera. E.g., XRHandsOffset
        /// </summary>
        [NonSerialized] private Transform xrHandsOffset = null;
        /// The initial local space position of the XR hands parent transform. E.g., XRHandsOffset
        /// </summary>
        [NonSerialized] private Vector3 initialXRHandsOffsetPos = Vector3.zero;
        /// <summary>
        /// The initial local space rotation of the XR hands parent transform. E.g., XRHandsOffset
        /// </summary>
        [NonSerialized] private Quaternion initialXRHandsOffsetRot = Quaternion.identity;

        [NonSerialized] private bool isInitialisedXR = false;

        [System.NonSerialized] internal static readonly List<XRInputSubsystem> xrInputSubsystemList = new List<XRInputSubsystem>();
        #endif
        ...
        private void ConfigureLeftHandXR()
        {
            ...
            {
                xrHandsOffset = leftHandTransformXR.parent;
        ...
        private void ConfigureRightHandXR()
        {
            ...
            {
                if (xrHandsOffset == null) { xrHandsOffset = rightHandTransformXR.parent; }
        ...
        /// <summary>
        /// Attempt to set the tracking origin mode.
        /// </summary>
        /// <returns></returns>
        private bool ConfigureXRInputSystems()
        {
            bool isSuccessful = true;
            SubsystemManager.GetSubsystems(xrInputSubsystemList);

            int numXRInputSystems = xrInputSubsystemList == null ? 0 : xrInputSubsystemList.Count;
            if (numXRInputSystems < 1) { isSuccessful  = false; }

            TrackingOriginModeFlags trackingOriginModeFlags = TrackingOriginModeFlags.Device;
            ...
            for (int xrInSysIdx = 0; xrInSysIdx < numXRInputSystems; xrInSysIdx++)
            {
                ...
                if (xrInputSubsystem != null)
                {
                    ...
                    else if (xrTrackingOriginMode != XRTrackingOriginMode.NotSet)
                    {
                        //Debug.Log("Original Tracking mode for (" + xrInSysIdx + ") " + xrInputSubsystem.GetTrackingOriginMode());

                        isSuccessful = xrInputSubsystem.TrySetTrackingOriginMode(GetXRTrackingOriginModeFlags(xrTrackingOriginMode)) && isSuccessful;

                        //Debug.Log("New Tracking mode for (" + xrInSysIdx + ") " + xrInputSubsystem.GetTrackingOriginMode());
                    }

                    if (isSuccessful && isRecentreOnInitXR)
                    {
        ...
        /// <summary>
        /// Attempt to initialise XR
        /// </summary>
        /// <param name="inputDevice"></param>
        /// <returns></returns>
        private IEnumerator InitaliseXR(UnityEngine.XR.InputDevice inputDevice)
        {
            ConfigureLeftHandXR();
            ...
            WaitForSeconds xrWait = new WaitForSeconds(0.1f);
            int initialiseXRCount = 0;
            ...
            while (!isInitialisedXR && initialiseXRCount < 30)
            {
                ...
                if (!isInitialisedXR)
                {
                    isInitialisedXR = ConfigureXRInputSystems();
                    initialiseXRCount++;
                }
            }
            ...
            if (isInitialisedXR)
            {
                if (firstPersonTransform1XR != null)
                {
                    xrCameraOffset = firstPersonTransform1XR.parent;
                    if (xrCameraOffset != null)
                    {
                        initialXRCamOffsetPos = xrCameraOffset.localPosition;
                        initialXRCamOffsetRot = xrCameraOffset.localRotation;

                        if (xrHandsOffset != null)
                        {
                            initialXRHandsOffsetPos = xrHandsOffset.localPosition;
                            initialXRHandsOffsetRot = xrHandsOffset.localRotation;
                        }

                        // If recentring, when we get the first non-zero value back from the headset,
                        // subtract the HMD rotation and position from the camera and hand parent gameobjects.
                        if (isRecentreOnInitXR)
                        {
                            // Distinguish between the old XR TrackPoseDriver and the one with the new Unity Input System.
                            UnityEngine.InputSystem.XR.TrackedPoseDriver xrCamTrackedPoseDriver;
                            if (firstPersonTransform1XR.TryGetComponent(out xrCamTrackedPoseDriver))
                            {
                                if (inputDevice.isValid)
                                {
                                    ...
                                    // Wait for first HMD input
                                    initialiseXRCount = 0;
                                    while (initialiseXRCount < 30 && hmdPos == Vector3.zero)
                                    {
                                        yield return xrWait;

                                        hmdPos = xrCamTrackedPoseDriver.positionAction.ReadValue<Vector3>();
                                        hmdRot = xrCamTrackedPoseDriver.rotationAction.ReadValue<Quaternion>();

                                        initialiseXRCount++;
                                    }

                                    if (hmdPos != Vector3.zero)
                                    {
                                        // Remove the initial HMD rotation
                                        // Remove the initial HMD position by rotating it by the subtracted rotation
                                        Quaternion subtractHMDRotY = Quaternion.Euler(0f, -hmdRot.eulerAngles.y, 0f);
                                        xrCameraOffset.SetLocalPositionAndRotation(initialXRCamOffsetPos - (subtractHMDRotY * hmdPos), subtractHMDRotY * initialXRCamOffsetRot);

                                        if (xrHandsOffset != null)
                                        {
                                            xrHandsOffset.SetLocalPositionAndRotation(initialXRHandsOffsetPos - (subtractHMDRotY * hmdPos), subtractHMDRotY * initialXRHandsOffsetRot);
                                        }
                                    }
                                }
                            }
        ...
        /// <summary>
        /// Get Unity.XR.TrackingOriginModeFlags from SSC XRTrackingOriginMode.
        /// Currently these should map directly with the Unity.XR equivalents
        /// </summary>
        /// <param name="trackingMode"></param>
        /// <returns></returns>
        private TrackingOriginModeFlags GetXRTrackingOriginModeFlags(XRTrackingOriginMode trackingMode)
        {
            return (TrackingOriginModeFlags)trackingMode;
        }
        ...
        {
            if (!isInitialisedXR && inputDevice.characteristics.HasFlag(InputDeviceCharacteristics.HeadMounted))
            {
                //Debug.Log("[DEBUG] PlayerInputModule HMD connected! name: " + inputDevice.name + " T:" + Time.time);
                StartCoroutine(InitaliseXR(inputDevice));
            }
        }
        ...
        #region Public API - Direct Keyboard

        /// <summary>
        /// Attempt to find the first matching custom player input
        /// </summary>
        /// <param name="keyCode"></param>
        /// <param name="mustBeButton"></param>
        /// <param name="customPlayerInput"></param>
        /// <returns></returns>
        public bool GetCustomPlayerInputDK (KeyCode keyCode, bool mustBeButton, out CustomPlayerInput customPlayerInput)
        {
            customPlayerInput = null;
            bool isFound = false;
            int numCustomInputs = customPlayerInputList == null ? 0 : customPlayerInputList.Count;
                                    
            for (int cIdx = 0; cIdx < numCustomInputs; cIdx++)
            {
                CustomPlayerInput _customInput = customPlayerInputList[cIdx];
                if (_customInput != null && (!mustBeButton || _customInput.isButton))
                {
                    if (_customInput.dkmPositiveKeycode == keyCode)
                    {
                        customPlayerInput = _customInput;
                        isFound = true;
                        break;
                    }
                }
        ...
        /// <summary>
        /// Attempt to get the first matching UIS or XR custom player input
        /// </summary>
        /// <param name="actionName"></param>
        /// <param name="mustBeButton"></param>
        /// <param name="mustBeEnabled"></param>
        /// <param name="customPlayerInput"></param>
        /// <returns>True if a match is found</returns>
        public bool GetCustomPlayerInputUIS (string actionName, bool mustBeButton, bool mustBeEnabled, out CustomPlayerInput customPlayerInput)
        {
            customPlayerInput = null;
            bool isFound = false;
            int numCustomInputs = customPlayerInputList == null ? 0 : customPlayerInputList.Count;
            ...
            {
                CustomPlayerInput _customInput = customPlayerInputList[cIdx];
                if (_customInput != null && (!mustBeButton || _customInput.isButton) && (!mustBeEnabled || _customInput.isButtonEnabled))
                {
                    UnityEngine.InputSystem.InputAction inputAction = _customInput.uisPositiveInputAction;

                    if (inputAction != null && !string.IsNullOrEmpty(inputAction.name) && inputAction.name == actionName)
                    {
                        customPlayerInput = _customInput;
                        isFound = true;
                        break;
        ...
        /// <param name="customPlayerInput"></param>
        /// <returns></returns>
        public bool GetCustomPlayerInputRW (int inputActionId , bool mustBeButton, out CustomPlayerInput customPlayerInput)
        {
            customPlayerInput = null;
            bool isFound = false;
            int numCustomInputs = customPlayerInputList == null ? 0 : customPlayerInputList.Count;
            ...
            for (int cIdx = 0; cIdx < numCustomInputs; cIdx++)
            {
                ...
                if (_customInput != null && (!mustBeButton || _customInput.isButton))
                {
                    if (_customInput.rwdPositiveInputActionId == inputActionId)
                    {
                        customPlayerInput = _customInput;
                        isFound = true;
                        break;
        ...
        /// called once at runtime for this ship, and then whenever the Actions are
