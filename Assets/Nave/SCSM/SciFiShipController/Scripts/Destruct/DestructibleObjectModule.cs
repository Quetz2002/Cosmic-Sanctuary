...
namespace SciFiShipController
{
    ...
    public class DestructibleObjectModule : MonoBehaviour
    {
        ...
        /// <summary>
        /// Whether damage type multipliers are used when calculating damage from projectiles or beams (or missiles and mines from X Pack 1).
        /// </summary>
        ...
        void OnDestroy()
        {
            if (isInitialised) { damageReceiver.callbackOnHit -= ApplyDamage; }
        }
        ...
        public virtual void Initialise()
        {
            if (!isInitialised)
            {
                if (!TryGetComponent(out damageReceiver)) { damageReceiver = gameObject.AddComponent<DamageReceiver>(); }
        ...
        public virtual void DestructObjectDelayed (float delayDuration)
        {
            if (delayDuration > 0f)
            {
                Invoke(nameof(DestructObject), delayDuration);
            }
            else { DestructObject(); }
        }
