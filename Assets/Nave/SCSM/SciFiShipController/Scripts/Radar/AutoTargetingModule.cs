    ...
    public class AutoTargetingModule : MonoBehaviour
    {
        ...
        private void UpdateShipWeaponTargets()
        {
            if (shipControlModule != null && shipControlModule.IsInitialised && shipControlModule.ShipIsEnabled())
            {
                ...
                #region AutoTargeting Turrets (all turrets) and fixed Missile weapons (isTargetsShownOnHUD [OFF])
                for (int wpIdx = 0; wpIdx < numWeapons; wpIdx++)
                {
                    Weapon weapon = shipControlModule.shipInstance.weaponList[wpIdx];

                    /// TODO - Missile Launcher (XPack1) uses a scoring system to allocate targets.
                    /// Here we assign the first one, which doesn't seem clever.
                    ...
                    {
                        bool isTurret = weapon.weaponTypeInt == Weapon.TurretProjectileInt || weapon.weaponTypeInt == Weapon.TurretBeamInt;
                        bool isMissileWithoutDisplayTargets = !isTurret && weapon.IsMissile && numDisplayTargets < 1;

                        if (isTurret || isMissileWithoutDisplayTargets)
                        {
                            ...
                            if (isNewTargetRequired && numBlips > 0)
                            {
                                ...
                                for (int bIdx = 0; bIdx < numBlips; bIdx++)
                                {
                                    ...
                                    #region Find the position of the target in turret space
                                    if (isTurret)
                                    {
                                    ...
                                    #region Check target is front of the Missile weapon

                                    if (isMissileWithoutDisplayTargets)
                                    {
                                        // Get the dot product between the weapon firing directin and the radar blip
                                        Vector3 _shipPos = shipControlModule.shipInstance.TransformPosition;
                                        Quaternion _shipRot = shipControlModule.shipInstance.TransformRotation;
                                        Vector3 _wsPosWeapon = _shipPos + (_shipRot * weapon.relativePosition);
                                        Vector3 _blipDirection = SSCMath.Normalise(_wsPosWeapon, blip.wsPosition);

                                        float _dp = Vector3.Dot(_shipRot * weapon.fireDirectionNormalised, _blipDirection);

                                        // Is the blip outside the 45 deg cone infront of the weapon?
                                        if (_dp < 0.5f) { continue; }
                                    }
                                    ...
                                    #region Ship and Damage Regions
                                    if (blip.shipControlModule != null)
                                    {
                                        // Check if this target is a ship or the damage region of a ship
                                        if (blip.guidHash == 0)
                                        {
                                            ...
                                            // Unlike turrets which need to be facing the target, fixed missile
                                            // weapons automatically become locked on the target.
                                            if (isMissileWithoutDisplayTargets) { weapon.isLockedOnTarget = true; }
                ...
                if (numDisplayTargets > 0 && isTargetsShownOnHUD)
                {
                    #region Allocate Display Targets

                    #region Reset Targets
                    ...
                    // OR (v1.5.6+) are not using a display target
                    for (int bIdx = 0; bIdx < numBlips; bIdx++)
                    {
                        ...
                        // Is this something that we always want to do? Maybe it should be optional...
                        ...
                        if (isTargetsShownOnHUD && !sscRadar.IsBlipInScreenViewPort(blip, shipDisplayModule.mainCamera, shipDisplayModule.ScreenResolution,
                                     shipDisplayModule.GetTargetsViewportSize, shipDisplayModule.GetTargetsViewportOffset)) { continue; }

                        #region Match Blips to Display Targets
                            ...
                            for (int sIdx = 0; sIdx < displayTarget.maxNumberOfTargets; sIdx++)
                            {
                                ...
                                if (targetRadarItem != null)
                                {
                                    ...
                                    if (isCheckLineOfSightNewTarget)
                                    {
                                        ...
                                        {
                                            /// TODO - check GC impact and revise GetLocation if necessary.
                                            /// TODO - attempt to avoid looking up Location twice 
                                            /// We "could" take risk and just set this to true but run
                                            /// risk location has been destroyed or removed (or is in wrong scene).
                    ...
                    for (int wpIdx = 0; wpIdx < numWeapons; wpIdx++)
                    {
                        ...
                        if (weapon != null && weapon.isAutoTargetingEnabled && (weapon.weaponTypeInt == Weapon.FixedProjectileInt || weapon.weaponTypeInt == Weapon.FixedBeamInt))
                        {
                            ...
                            if (fixedWeaponBestTargetRadarItem != null)
                            {
                                ...
                                // Is it a ship or ship damage region?
                                if (fixedWeaponBestTargetRadarItem.shipControlModule != null)
                                {
                                    // Check if this is a ship rather than a ship damage region target
                                ...
                                // Is it a gameobject or a SurfaceTurret (without a Location) or
                                // a Launcher (from XPack1 without a Location)?
                                ...
                                // Unlike turrets which need to be facing the target, Missile on fixed projectile weapons
                                // automatically become locked on the target.
                                if (weapon.IsMissile) { weapon.isLockedOnTarget = true; }
                            }
