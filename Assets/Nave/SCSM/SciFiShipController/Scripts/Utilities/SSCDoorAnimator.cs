...
namespace SciFiShipController
{
    ...
    public class SSCDoorAnimator : MonoBehaviour
    {
        ...
        /// <summary>
        /// Attempt to open all doors instantly rather than playing the animation.
        /// Do not play any audio. Ignore lock status. Do not invoke any closing events.
        ...
        public void OpenDoorsAllInstantly (int stateId)
        {
            if (_animator != null && numOpenParms > 0)
            {
                try
                {
                    ...
                    else
                    {
                        for (int doorIndex = 0; doorIndex < numOpenParms; doorIndex++)
                        {
                            _animator.SetBool(isOpenHash[doorIndex], true);
                        }
                    }
                }
                catch (Exception ex)
                {
                    Debug.LogWarning("[ERROR] SSCDoorAnimator.OpenDoorsAllInstantly() on " + name + " " + ex.Message);
                }
            }
        }
        ...
        /// <summary>
        /// Attempt to close all doors instantly rather than playing the animation.
        /// Do not play any audio. Ignore lock status. Do not invoke any closing events.
        /// To get the the stateId call SSCUtils.GetAnimationStateID(stateName). Cache the stateId to avoid GC.
        /// e.g., int stateId = SSCUtils.GetAnimationStateID("RearRamp_Raise");
        /// </summary>
        /// <param name="stateId"></param>
        public void CloseDoorsAllInstantly (int stateId)
        {
            if (_animator != null)
            {
                try
                {
                    ...
                    if (useTriggers)
                    {
                        for (int doorIndex = 0; doorIndex < numCloseParms && doorIndex < numOpenParms; doorIndex++)
                        {
                            if (IsDoorOpenByIndex(doorIndex))
                            {
                                _animator.ResetTrigger(isOpenHash[doorIndex]);
                    ...
                    else
                    {
                        for (int doorIndex = 0; doorIndex < numOpenParms; doorIndex++)
                        {
                            _animator.SetBool(isOpenHash[doorIndex], false);
                        }
                    }
                }
                catch (Exception ex)
                {
                    Debug.LogWarning("[ERROR] SSCDoorAnimator.CloseDoorsAllInstantly() on " + name + " " + ex.Message);
                }
            }
        }
        ...
        /// <summary>
        /// Attempt to toggle the lock status of all the doors based on their current locked status.
        /// </summary>
        public void ToggleLockDoors()
        {
            ...
            for (int doorIndex = 0; doorIndex < numOpenParms; doorIndex++)
            {
                isLockedStatuses[doorIndex] = !isLockedStatuses[doorIndex];
            }
        }

        /// <summary>
        /// Attempt to toggle the lock status of the specific door or set of doors given the zero-based
        /// index in the array of open param names.
        /// </summary>
        /// <param name="doorIndex"></param>
        public void ToggleLockDoors (int doorIndex)
        {
            if (!IsInitialised) { VerifyLockArray(); }
            ...
            {
                Debug.LogWarning("[ERROR] SSCDoorAnimator.ToggleLockDoors() - the doorIndex of " + doorIndex + " is invalid. Check the number of openParamNames.");
            }
            #endif
        }
