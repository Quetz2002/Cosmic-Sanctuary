    ...
    public class AIState
    {
        ...
        private static void DogfightState (AIStateMethodParameters stateMethodParameters)
        {
            if (stateMethodParameters.targetShip != null)
            {
                ...
                // +DHS 6/10/25
                // Get the relative velocity between the target ship and this ship
                Vector3 relativeTargetShipVelocity = stateMethodParameters.targetShip.WorldVelocity - stateMethodParameters.shipControlModule.shipInstance.WorldVelocity;
                float approxRealInterceptionTime = 1000f;
                float relativeTargetShipVelocitySqrMagnitude = relativeTargetShipVelocity.sqrMagnitude;
                // We can only calculate interception time if the ship is moving towards us
                float interceptionDotProduct = Vector3.Dot(relativeTargetShipVelocity, fromTargetShipVector);
                if (interceptionDotProduct > 0f)
                {
                    // We can only calculate interception time accurately if there is a significant
                    // relative velocity (i.e. > 10 m/s)
                    if (relativeTargetShipVelocitySqrMagnitude > 100f)
                    {
                        // Calculates distance divided by relative velocity divided by cos(theta) of the angle
                        // between them, which is good enough for an approximate intercept time
                        approxRealInterceptionTime = distToTargetShip * distToTargetShip / interceptionDotProduct;
                    }
                    else
                    {
                        // If relative velocity is low, assume we are moving at 10 m/s towards target ship
                        // This addresses problem where ships are close together
                        approxRealInterceptionTime = distToTargetShip / 10f;
                    }
                }

                // TODO use this new intercept time - probably remove other times
                // TODO TEST ONLY CODE
                approxPursueInterceptionTime = approxRealInterceptionTime;
                approxEvadeInterceptionTime = approxRealInterceptionTime;
                ...
                // Condition 1: We are "in front" of the target ship
                // Condition 2: We are currently facing away from the target ship
                // Condition 3: We will be intercepted in less than 3 seconds
                ...
                // Priority #4: Avoid collision with target ship when we get close
                // Condition 1: The target ship is "in front" of us
                // Condition 2: We will intercept the other ship in less than 2 seconds
                if (Vector3.Dot(fromTargetShipVector, stateMethodParameters.shipControlModule.shipInstance.TransformForward) < 0f &&
                    approxRealInterceptionTime < 2f)
                {
                    stateMethodParameters.aiBehaviourInputsList[3].behaviourType = AIBehaviourInput.AIBehaviourType.CustomUnblockCylinder;
                    stateMethodParameters.aiBehaviourInputsList[3].targetPosition = stateMethodParameters.shipControlModule.shipInstance.TransformPosition + fromTargetShipVector;
                    stateMethodParameters.aiBehaviourInputsList[3].targetForwards = fromTargetShipVector;
                    stateMethodParameters.aiBehaviourInputsList[3].targetRadius = 30f + stateMethodParameters.shipAIInputModule.shipRadius;
                    stateMethodParameters.aiBehaviourInputsList[3].weighting = 1f;
                }

                // Priority #5: Pursue/seek target ship
                if (approxPursueInterceptionTime > 3f && approxPursueInterceptionTime < 10f)
                {
                    stateMethodParameters.aiBehaviourInputsList[4].behaviourType = AIBehaviourInput.AIBehaviourType.CustomPursuitArrival;
                }
                else
                {
                    stateMethodParameters.aiBehaviourInputsList[4].behaviourType = AIBehaviourInput.AIBehaviourType.CustomSeekMovingArrival;
                }
                stateMethodParameters.aiBehaviourInputsList[4].targetPosition = stateMethodParameters.targetShip.TransformPosition;
                stateMethodParameters.aiBehaviourInputsList[4].targetVelocity = stateMethodParameters.targetShip.WorldVelocity;
                stateMethodParameters.aiBehaviourInputsList[4].useTargetingAccuracy = true;
                stateMethodParameters.aiBehaviourInputsList[4].weighting = 1f;
