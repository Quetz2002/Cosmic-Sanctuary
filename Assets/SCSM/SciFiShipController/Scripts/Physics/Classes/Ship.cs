...
namespace SciFiShipController
{
    ...
    public class Ship
    {
        ...
        /// <summary>
        /// [INTERNAL ONLY]
        /// Use shipControlModule.SetMaxAngularVelocity(..) instead.
        /// If greater than 0, this will override the Default Unity Physics Settings with the maximum angular velocity in degrees per second.
        /// Typically, values below 180 work the best.
        /// </summary>
        [Range(0f, 3600f), Delayed] public float maxAngularVelocity;
        ...
        /// When disabling at runtime, call ReinitialisePitchRollMatchVariables()
        /// to avoid issues when it is re-enabled.
        /// </summary>
        ...
        /// </summary>
        [Range(1f, 4.5f), Delayed] public float pitchDamping = 1f;
        /// <summary>
        ...
        /// If changed at runtime, must call ReinitialisePitchRollMatchVariables().
        /// </summary>
        [Range(1f, 4.5f), Delayed] public float rollDamping = 1f;
        /// <summary>
        ...
        /// <summary>
        /// The time, in seconds, that variable pitch acceleration will be in effect after receiving pitch input. Currently only applies to arcade model.
        /// </summary>
        [Range(0f, 0.5f)] public float variablePitchDuration;
        /// <summary>
        /// The time, in seconds, that variable roll acceleration will be in effect after receiving roll input. Currently only applies to arcade model.
        /// </summary>
        [Range(0f, 0.5f)] public float variableRollDuration;
        /// <summary>
        /// The time, in seconds, that variable yaw acceleration will be in effect after receiving yaw input. Currently only applies to arcade model.
        /// </summary>
        [Range(0f, 0.5f)] public float variableYawDuration;
        /// <summary>
        ...
        /// <summary>
        /// Whether damage multipliers are used when calculating damage from projectiles or beams (or missiles and mines from X Pack 1).
        /// </summary>
        ...
        private float variablePitchTimer = 0f, variablePitchAccel = 0f;
        private float variableRollTimer = 0f, variableRollAccel = 0f;
        private float variableYawTimer = 0f, variableYawAccel = 0f;
        private Vector3 prevPilotMomentInput = Vector3.zero;
        #endregion
        ...
        public Ship (Ship ship)
        {
            ...
            else
            {
                ...
                this.variablePitchDuration = ship.variablePitchDuration;
                this.variableRollDuration = ship.variableRollDuration;
                this.variableYawDuration = ship.variableYawDuration;
        ...
        /// <summary>
        /// Get the variable acceleration for an axis.
        /// Assumes the variableDuration > 0.001.
        /// </summary>
        /// <param name="fixedAcceleration"></param>
        /// <param name="variableDuration"></param>
        /// <param name="curPilotMomentInput"></param>
        /// <param name="prevPilotMomentInput"></param>
        /// <param name="timer"></param>
        /// <param name="deltaTime"></param>
        /// <returns></returns>
        private float GetVariableAcceleration(float fixedAcceleration, float variableDuration, float curPilotMomentInput, float prevPilotMomentInput, ref float timer, float deltaTime)
        {
            float variableAccel = fixedAcceleration;

            if (curPilotMomentInput != 0f && fixedAcceleration > 0f)
            {
                // was there any input in the last frame?
                if (prevPilotMomentInput == 0f)
                {
                    // Set to the amount of easing time.
                    timer = variableDuration;
                }
                else if (timer <= 0f)
                {
                    timer = 0f;
                ...
                if (timer > 0f)
                {
                    variableAccel = Mathf.Lerp(0.001f, fixedAcceleration, timer / variableDuration);
            ...
            {
                variableAccel = fixedAcceleration;
            }
        ...
        public void SetClassDefaults()
        {
            ...
            this.variablePitchDuration = 0f;
            this.variableYawDuration = 0f;
            this.variableRollDuration = 0f;
        ...
        /// Call this when disabling limitPitchAndRoll at runtime and when
        /// modifying pitch and roll damping.
        /// </summary>
        ...
        /// <summary>
        /// Re-initialise pitch, roll, and yaw variables
        /// that are not used with LimitPitchAndRoll.
        /// </summary>
        public void ReinitialisePitchRollYaw()
        {
            variablePitchTimer = 0f;
            variableRollTimer = 0f;
            variableYawTimer = 0f;
        }
        ...
        public void CalculateForceAndMoment(ref Vector3 localResultantForce, ref Vector3 localResultantMoment, float physicsDeltaTime)
        {
            ...
            if (limitPitchAndRollEnabled)
            {
                ...
                maxFrameRollDelta = turnRollSpeed * _deltaTime;
                ...
                //Debug.Log("Target pitch: " + targetLocalPitch.ToString("00.00") + ", target roll: " + targetLocalRoll.ToString("00.00") + " rollAmount: " + rollAmount.ToString("0.00") + " T:" + Time.time);
            ...
            if (!shipPhysicsModelPhysicsBased)
            {
                // Durations less that 10ms would have no noticable effect.
                variablePitchAccel = variablePitchDuration < 0.01f ? arcadePitchAcceleration : GetVariableAcceleration(arcadePitchAcceleration, variablePitchDuration, pilotMomentInput.x, prevPilotMomentInput.x, ref variablePitchTimer, _deltaTime);
                variableYawAccel = variableYawDuration < 0.01f ? arcadeYawAcceleration : GetVariableAcceleration(arcadeYawAcceleration, variableYawDuration, pilotMomentInput.y, prevPilotMomentInput.y, ref variableYawTimer, _deltaTime);
                variableRollAccel = variableRollDuration < 0.01f ? arcadeRollAcceleration : GetVariableAcceleration(arcadeRollAcceleration, variableRollDuration, pilotMomentInput.z, prevPilotMomentInput.z, ref variableRollTimer, _deltaTime);
                ...
                localResultantMoment.x += ((momentInput.x * variablePitchAccel) + arcadeMomentVector.x) * Mathf.Deg2Rad * rBodyInertiaTensor.x;
                localResultantMoment.y += ((momentInput.y * variableYawAccel) + arcadeMomentVector.y) * Mathf.Deg2Rad * rBodyInertiaTensor.y;
                localResultantMoment.z += ((momentInput.z * variableRollAccel) + arcadeMomentVector.z) * Mathf.Deg2Rad * rBodyInertiaTensor.z * -1f;
            ...
            prevPilotMomentInput = pilotMomentInput;
                    ...
                    if (weaponFiringButtonInt > 0 && weapon.health > 0f && weapon.currentPerformance > 0f)
                    {
                        ...
                        else if (shipDamageModelIsSimple || weapon.damageRegionIndex < 0 || weapon.Health > 0f)
                        {
                            ...
                            else if (!isReadyToFire && weaponFiringButtonInt == Weapon.FiringButtonAutoInt && weaponTypeInt == Weapon.FixedProjectileInt && weapon.IsMissile)
                            {
                                ...
                                // NOTE: Missiles from XPack 1 have to be locked on target
                                isReadyToFire = weapon.isLockedOnTarget && (!weapon.checkLineOfSight || weapon.isTargetLocation || WeaponHasLineOfSight(weapon));

                                //Debug.Log("[DEBUG] Autofire isReady " + isReadyToFire + " IsMissile: " + weapon.IsMissile);
                            }
