...
namespace SciFiShipController
{
    ...
    public class ShipControlModule : MonoBehaviour
    {
        ...
        /// <summary>
        /// Attempt to get or set the maximum angular velocity
        /// </summary>
        public float MaxAngularVelocity { get { return isInitialised ? shipInstance.maxAngularVelocity : 0f; } set { SetMaxAngularVelocity(value); } }
        ...
        public static string SSCVersion { get { return "1.5.8"; } }
        ...
        /// <summary>
        /// Attempt to set the Maximum Angular Velocity in degrees per second.
        /// </summary>
        /// <param name="maxDegreesPerSecond"></param>
        public void SetMaxAngularVelocity (float maxDegreesPerSecond)
        {
            // The RigidBody stores the value in Radians per second. We show it in Degrees per second
            if (rBody != null)
            {
                if (maxDegreesPerSecond > 0f)
                {
                    rBody.maxAngularVelocity = maxDegreesPerSecond * Mathf.Deg2Rad;
                }
                else
                {
                    maxDegreesPerSecond = 0f;
                    rBody.maxAngularVelocity = Physics.defaultMaxAngularSpeed;
                }
            }
            else if (shipInstance != null)
            {
                shipInstance.maxAngularVelocity = maxDegreesPerSecond < 0f ? 0f : maxDegreesPerSecond;
            }
        }
        ...
        public virtual void InitialiseShip()
        {
            if (!isInitialised)
            {
                ...
                SetMaxAngularVelocity(shipInstance.maxAngularVelocity);
        ...
        public void ReinitialiseThrusterEffects()
        {
            ...
            if (arrayLength > 0)
            {
                ...
                // One ThrusterEffects component is added to the thruster effects parent gameobject (if there is one) for each Thruster in the scene.
