...
namespace SciFiShipController
{
    ...
    public class Celestials : MonoBehaviour
    {
        ...
        /// URP VR
        /// </summary>
        [HideInInspector] public bool isVR = false;
        ...
        #region Public Variables - URP
        #if SSC_URP
        [HideInInspector] public Material skyMaterialURP = null;
        #endif
        #endregion
        ...
        [System.NonSerialized] protected bool isCamera2Initialised = false;
        [System.NonSerialized] protected SSCRandom sscRandom = null;
        ...
        [System.NonSerialized] protected float minCelestialCameraDistance = 0.2f;
        [System.NonSerialized] protected int timingTypeInt = (int)TimingType.Auto;

        #if SSC_HDRP
        [System.NonSerialized] protected float distanceScale = 1000f;
        #else
        [System.NonSerialized] protected float distanceScale = 1f;
        #endif

        #endregion

        #region Protected Variables - HDRP or URP
        #if SSC_HDRP || SSC_URP
        /// <summary>
        /// The stars colour range from
        /// </summary>
        [HideInInspector, SerializeField] protected Color colourFrom = Color.white;

        /// <summary>
        /// The stars colour range to
        /// </summary>
        [HideInInspector, SerializeField] protected Color colourTo = Color.white;

        /// <summary>
        /// The amount of fade to apply to the stars
        /// </summary>
        [HideInInspector, SerializeField, Range(0f, 1f)] protected float fadeStars = 0.6f;

        /// <summary>
        /// The density of the stars
        /// </summary>
        [HideInInspector, SerializeField, Range(0f, 1f)] protected float density = 0.4f;

        /// <summary>
        /// The starfield to be displayed
        /// </summary>
        [HideInInspector, SerializeField, Range(0f, 1f)] protected float starfield = 0.5f;

        /// <summary>
        /// The normalised position of the horizon on the y-axis
        /// </summary>
        [HideInInspector, SerializeField, Range(0f, 1f)] protected float horizon = 1f;
        ...
        #if SSC_HDRP
        [System.NonSerialized] protected HDRISky hdriSkyOverride;
        [System.NonSerialized] protected Material skyMaterial;
        [System.NonSerialized] protected CustomRenderTexture tempCustomRT;
        #endif
        #endregion
        ...
        public void Initialise()
        {
            ...
            {
                #if !SSC_URP
                Debug.LogWarning("[ERROR] Celestials.Initialise - VR stars requires URP");
                #else

                if (skyMaterialURP == null)
                {
                    Debug.LogWarning("[ERROR] Celestials.Initialise - the Sky Material for " + name + " in scene " + gameObject.scene.name + " is not set in the inspector for URP VR");
                }
                else if (!SSCUtils.GetShaderPropertyId(skyMaterialURP, "_Horizon", out matHorizonId))
                {
                    Debug.LogWarning("[ERROR] Celestials.Initialise - the sky material (" + skyMaterialURP.name + ") does not have a property called Horizon for " + name + " in scene " + gameObject.scene.name);
                }
                else if (!SSCUtils.GetShaderPropertyId(skyMaterialURP, "_HasHorizon", out matHasHorizonId))
                {
                    Debug.LogWarning("[ERROR] Celestials.Initialise - the sky material (" + skyMaterialURP.name + ") does not have a property called HasHorizon for " + name + " in scene " + gameObject.scene.name);
                }
                else if (!SSCUtils.GetShaderPropertyId(skyMaterialURP, "_ColourFrom", out matColourFromId))
                {
                    Debug.LogWarning("[ERROR] Celestials.Initialise - the sky material (" + skyMaterialURP.name + ") does not have a property called ColourFrom for " + name + " in scene " + gameObject.scene.name);
                }
                else if (!SSCUtils.GetShaderPropertyId(skyMaterialURP, "_ColourTo", out matColourToId))
                {
                    Debug.LogWarning("[ERROR] Celestials.Initialise - the sky material (" + skyMaterialURP.name + ") does not have a property called ColourTo for " + name + " in scene " + gameObject.scene.name);
                }
                else if (!SSCUtils.GetShaderPropertyId(skyMaterialURP, "_Density", out matDensityId))
                {
                    Debug.LogWarning("[ERROR] Celestials.Initialise - the sky material (" + skyMaterialURP.name + ") does not have a property called Density for " + name + " in scene " + gameObject.scene.name);
                }
                else if (!SSCUtils.GetShaderPropertyId(skyMaterialURP, "_Starfield", out matStarfieldId))
                {
                    Debug.LogWarning("[ERROR] Celestials.Initialise - the sky material (" + skyMaterialURP.name + ") does not have a property called Starfield for " + name + " in scene " + gameObject.scene.name);
                }
                else if (!SSCUtils.GetShaderPropertyId(skyMaterialURP, "_FadeStars", out matFadeStarsId))
                {
                    Debug.LogWarning("[ERROR] Celestials.Initialise - the sky material (" + skyMaterialURP.name + ") does not have a property called FadeStars for " + name + " in scene " + gameObject.scene.name);
                }
                else
                {
        ...
        void FixedUpdate()
        {
            if (isInitialised && !isVR && timingTypeInt == timingTypeIntAuto)
            {
...
#if SSC_HDRP
#if UNITY_2021_3_OR_NEWER
            // For consistency, create the same hierachy as BiRP and URP without VR
        ...
        protected Transform CreatePlanet(SSCCelestial sscCelestial)
        {
            ...
            if (sscCelestial != null)
            {
                ...
                #elif SSC_URP
                if (isVR) { scaleFactor *= distanceScale; }
                #endif
        ...
        {
            #if !SSC_HDRP
            if (isInitialised && !isVR && timingTypeInt == timingTypeIntManual)
            {
                UpdateCelestialsRotation();
            }
            #endif
        ...
        public void DisableHorizon()
        {
            ...
            #elif SSC_URP
            if (isInitialised && isVR && matHasHorizonId != 0)
            {
                skyMaterialURP.SetFloat(matHasHorizonId, 0f);
            }
            #endif
        }
        ...
        public void EnableHorizon()
        {
            ...
            if (isInitialised && isVR && matHasHorizonId != 0)
            {
                skyMaterialURP.SetFloat(matHasHorizonId, 1f);
            }
            #endif
        }
        ...
        public float GetCelestialDistance(SSCCelestial celestial, bool forceRecalc = false)
        {
            if (celestial != null) // && !celestial.isHidden)
            {
                ...
                #endif
                #if SSC_URP
                else if (isInitialised && isVR && camera1 != null)
                {
                    // Planet (A) Camera (B). Direction = (B-A).normalized
                    Vector3 _lookVector = camera1.transform.position - celestial.celestialTfrm.position;
                    celestial.currentCelestialDistance = _lookVector.magnitude - minCelestialCameraDistance;
        ...
        public void HideStars()
        {
            ...
            if (isVR)
            {
                ...
                if (isInitialised && matFadeStarsId != 0)
                {
                    skyMaterialURP.SetFloat(matFadeStarsId, 1f);
                }
                #endif
            }
            else if (starMRenderer != null)
            {
        ...
        /// <summary>
        /// Attempt to set the fade stars value in HDRP or URP VR.
        /// Has no effect in BiRP or URP without VR.
        /// </summary>
        ...
        public void SetFadeStars (float newValue)
        {
            ...
            #elif SSC_URP
            if (isInitialised && isVR && matFadeStarsId != 0)
            {
                skyMaterialURP.SetFloat(matFadeStarsId, newValue);
            }
            #endif
        }

        /// <summary>
        /// Attempt to set the star colour range from in HDRP or URP VR.
        /// Has no effect in BiRP or URP without VR.
        /// </summary>
        /// <param name="newValue"></param>
        public void SetStarColourFrom (Color newColour)
        {
            ...
            #elif SSC_URP
            if (isInitialised && isVR && matColourFromId != 0)
            {
                skyMaterialURP.SetColor(matColourFromId, newColour);
            }
            #endif
        }

        /// <summary>
        /// Attempt to set the star colour range to in HDRP or URP VR.
        /// Has no effect in BiRP or URP without VR.
        /// </summary>
        /// <param name="newValue"></param>
        public void SetStarColourTo (Color newColour)
        {
            ...
            #elif SSC_URP
            if (isInitialised && isVR && matColourToId != 0)
            {
                skyMaterialURP.SetColor(matColourToId, newColour);
            }
            #endif
        }

        /// <summary>
        /// Attempt to set the star density in HDRP or URP VR.
        /// Has no effect in BiRP or URP without VR.
        /// </summary>
        /// <param name="newValue"></param>
        public void SetStarDensity (float newValue)
        {
            ...
            #elif SSC_URP
            if (isInitialised && isVR && matDensityId != 0)
            {
                skyMaterialURP.SetFloat(matDensityId, newValue);
            }
            #endif
        }

        /// <summary>
        /// Attempt to set the starfield in HDRP or URP VR.
        /// Has no effect in BiRP or URP without VR.
        /// </summary>
        /// <param name="newValue"></param>
        public void SetStarfield (float newValue)
        {
            ...
            #elif SSC_URP
            if (isInitialised && isVR && matStarfieldId != 0)
            {
                skyMaterialURP.SetFloat(matStarfieldId, newValue);
            }
            #endif
        }

        /// <summary>
        /// Attempt to set the horizon in HDRP or URP VR.
        /// Has no effect in BiRP or URP without VR.
        /// </summary>
        /// <param name="newValue"></param>
        public void SetHorizon (float newValue)
        {
            ...
            #elif SSC_URP
            if (isInitialised && isVR && matHorizonId != 0)
            {
                skyMaterialURP.SetFloat(matHorizonId, newValue);
            }
            #endif
        }
        ...
        public void ShowStars()
        {
            ...
            if (isVR)
            {
                ...
                if (isInitialised && matFadeStarsId != 0)
                {
                    skyMaterialURP.SetFloat(matFadeStarsId, 0f);
                }
                #endif
            }
            else 
        ...
        /// Has no effect in URP with VR or HDRP.
        /// </summary>
