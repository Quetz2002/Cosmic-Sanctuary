...
namespace SciFiShipController
{
    ...
    public class ShipControlModuleEditor : Editor
    {
        ...
        private readonly static GUIContent maxAngularVelocityContent = new GUIContent(" Max Angular Velocity", "If greater than 0, this " +
           " will override the Default Unity Physics Settings with the maximum angular velocity in degrees per second. Typically, values " +
           "below 180 work the best.");
        ...
        private readonly static GUIContent variableAccelerationContent = new GUIContent(" Variable Acceleration");
        private readonly static GUIContent variablePitchDurationContent = new GUIContent("  Pitch Accel. Duration", "The time, in seconds, " +
            "that variable pitch acceleration will be in effect after receiving pitch input.");
        private readonly static GUIContent variableRollDurationContent = new GUIContent("  Roll Accel. Duration", "The time, in seconds, " +
           "that variable roll acceleration will be in effect after receiving roll input.");
        private readonly static GUIContent variableYawDurationContent = new GUIContent("  Yaw Accel. Duration", "The time, in seconds, " +
            "that variable yaw acceleration will be in effect after receiving yaw input.");
        ...
        private readonly static GUIContent useDamageMultipliersContent = new GUIContent(" Use Damage Multipliers", "Whether " +
            "damage type multipliers are used when calculating damage from projectiles or beams (or missiles and mines from X Pack 1).");
        ...
        private readonly static GUIContent weaponTurretMinYContent = new GUIContent("Turret Min. Y", "The minimum angle on the local y-axis the turret can rotate to. If you cannot see the zooming in or out in the editor. Sometime turning OFF Gizmos in the scene view can make it more visible.");
        private readonly static GUIContent weaponTurretMaxYContent = new GUIContent("Turret Max. Y", "The maximum angle on the local y-axis the turret can rotate to. If you cannot see the zooming in or out in the editor. Sometime turning OFF Gizmos in the scene view can make it more visible.");
        private readonly static GUIContent weaponTurretMinXContent = new GUIContent("Turret Min. X", "The minimum angle on the local x-axis the turret can elevate to. If you cannot see the zooming in or out in the editor. Sometime turning OFF Gizmos in the scene view can make it more visible.");
        private readonly static GUIContent weaponTurretMaxXContent = new GUIContent("Turret Max. X", "The maximum angle on the local x-axis the turret can elevate to. If you cannot see the zooming in or out in the editor. Sometime turning OFF Gizmos in the scene view can make it more visible.");
        #if SCSM_SSCXP1
        ...
        private SerializedProperty maxAngularVelocityProp;
        ...
        protected virtual void DrawControlSettings(bool isArcadeModel, bool isPhysicsModel)
        {
            ...
            if (EditorGUI.EndChangeCheck() && EditorApplication.isPlaying && !limitPitchAndRollProp.boolValue)
            {
        ...
        protected virtual void DrawPhysicsSettings(bool isArcadeModel)
        {
            if (shipInstanceProp != null)
            {
                ...
                #region Max Angular Velocity
                maxAngularVelocityProp = shipInstanceProp.FindPropertyRelative("maxAngularVelocity");
                EditorGUI.BeginChangeCheck();
                EditorGUILayout.PropertyField(maxAngularVelocityProp, maxAngularVelocityContent);
                if (EditorGUI.EndChangeCheck() && Application.isPlaying)
                {
                    serializedObject.ApplyModifiedProperties();
                    shipControlModule.SetMaxAngularVelocity(maxAngularVelocityProp.floatValue);
                    serializedObject.Update();
                }
                #endregion
                ...
                if (isArcadeModel)
                {
                    ...
                    EditorGUILayout.LabelField(variableAccelerationContent);
                    SSCEditorHelper.DrawSSCPropertyIndent(10, shipInstanceProp.FindPropertyRelative("variablePitchDuration"), variablePitchDurationContent, defaultEditorLabelWidth);
                    SSCEditorHelper.DrawSSCPropertyIndent(10, shipInstanceProp.FindPropertyRelative("variableRollDuration"), variableRollDurationContent, defaultEditorLabelWidth);
                    SSCEditorHelper.DrawSSCPropertyIndent(10, shipInstanceProp.FindPropertyRelative("variableYawDuration"), variableYawDurationContent, defaultEditorLabelWidth);
